...
def process_files(dir, framerate, starting_frame, frames, filetype, animate, reverse, mirror_list, info):
    ...
    if len(files) > 0:
        ...
        # if only file found is an unsorted image
        if fi == len(files) and len(collected_files) == 1:
            sort_unsorted_image(dir, f, nametomatch)
        ...
        fAmt = len(files)
        for i, f in enumerate(files[fi:], start=fi):
...
def create_animation_file(dirpath, dirname, framerate, frames, filetype, starting_frame, mirror_list, reverse, diroutpath, info):
    ...
    print("Animating: %s\nStarting frame %d\nEnd Frame: %d\nFile List Length: %d\n\nSaving file to %s" % (animator.name, animator.starting_frame, animator.starting_frame + animator.frames - 1, animator.length, outpath))
...
class MLAnimator:
        ...
        try:
            while True:
                ...
                if not frames in ("", None):
                    if not frames.isnumeric(): return False
                    frames = int(frames)
                    if frames < 1:
                ...
                if self.set_starting_frame(max):
                    if newframes:
    ...
    def set_starting_frame(self, max):
        print("\nSelect starting frame (MAX #: %d)\n(Entering nothing sets starting frame as high as possible | Enter 's' to go back to set frame amount)\n" % max)
        try:
            while True:
                starting_frame = input("Enter starting frame selection: ").strip()
                ...
                if starting_frame > max:
                    print("%s Error: Cannot render %d frames from starting image %d (Image amount: %d)" % (self.name, self.length - starting_frame, starting_frame, self.length))
                    continue
    ...
    def check_valid_frames(self):
        if self.frames == None: # default frame count is distance from starting_frame to highest numbered frame.
            if self.starting_frame == None: # both default settings mean to use all the frames
                self.starting_frame = 1
                self.frames = self.length
                return True
        if self.frames >= self.length:
            select = input("Selected too many frames; set to max frame amount %d?" % (self.length))
            if select.strip().lower() in ("yes", "y"):
                self.starting_frame = 1
                self.frames = self.length
                return True
            return False
        if self.starting_frame == None: # default starting frame is highest numbered image minus the amount of frames
